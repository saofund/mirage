...
def mat(c, metallic=0.0, roughness=0.5):
    return {"color": list(c), "metallic": metallic, "roughness": roughness}
...
SHADE   = mat((0.98, 0.93, 0.80), 0.0, 0.6)    # warm lampshade (reads lit)
...
def trace(prog, png, w=1400, h=900, spp=240, denoise=5, threads=16):
    ...
    subprocess.run([str(RENDER), "--oplog", str(jp), "--out", str(ppm), "--w", str(w), "--h", str(h),
                    ...
                    "--sun", "0.95", "--env", "0.40", "--exposure", "0.74", "--sun-dir", "0.3", "0.7", "0.5",
...
def main():
    ...
    if "--preview" in sys.argv:
        ...
        trace(p, GALLERY / "grand_interior.png", w=1500, h=980, spp=300, threads=140)
