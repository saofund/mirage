...
def sample(rng, camera="orbbec640", domain="wide"):
    ...
    d_cap = _lerp(rng, 0.068, 0.090)
    # The rib scales with the cap rather than varying freely: across ninety cars the grip
    # is always about two thirds of the diameter and about a third of that across, and
    # sampling the two independently makes caps that no manufacturer has ever made.
    rib_len = d_cap * _lerp(rng, 0.60, 0.78)
    rib_w = rib_len * _lerp(rng, 0.34, 0.52)
    v = dict(
        camera=camera,
        d_cap=d_cap, flange=_lerp(rng, 0.007, 0.013),
        rib_len=rib_len, rib_w=rib_w,
        rib_h=_lerp(rng, 0.008, 0.016), rib_draft=_lerp(rng, 0.55, 0.82),
        ...
        # the pocket
        d_well=d_cap + _lerp(rng, 0.016, 0.048),
        depth_well=_lerp(rng, 0.028, 0.080),
        ...
        door_open=_lerp(rng, 72.0, 118.0),
        door_w=_lerp(rng, 0.150, 0.205), door_h=_lerp(rng, 0.140, 0.195),
        paint=str(rng.choice(M.PAINT_NAMES)),
        tether=bool(rng.random() < 0.55),
        alu=bool(rng.random() < 0.08),
        # lighting
        sun=_lerp(rng, 0.5, 4.2), env=_lerp(rng, 0.25, 1.5),
        sun_az=_lerp(rng, 0.0, 360.0), sun_el=_lerp(rng, 12.0, 82.0),
        sky_flat=float(rng.random() < 0.55),
        exposure=float(math.exp(rng.normal(0.0, 0.30))),
    ...
    if domain == "prod":
        v.update(dist=_lerp(rng, 0.41, 0.48), obliq=_lerp(rng, 0.0, 14.0))
    elif domain == "orbbec":
        v.update(dist=_lerp(rng, 0.28, 0.64), obliq=_lerp(rng, 0.0, 42.0))
    else:                                        # the union, widened at both ends
        v.update(dist=float(np.exp(rng.uniform(math.log(0.26), math.log(1.35)))),
                 obliq=_lerp(rng, 0.0, 55.0))
...
def build(v):
    ...
    prog = prog.place(obj=P.panel(size=0.46, hole_d=2 * rim_r, thick=0.009, material=paint),
                      at=(0.0, 0.0, 0.0))
    ...
    cap_prog = P.cap(d=v["d_cap"], flange=v["flange"], rib_len=v["rib_len"], rib_w=v["rib_w"],
                     ...
                     skirt=v["skirt"], neck_d=v["d_cap"] * 0.62, material=cap_mat)
    prog = prog.place(obj=cap_prog, at=tuple(cap_c),
                      rotate=(v["tilt_x"], v["tilt_y"], v["cap_spin"]))
    ...
    prog = prog.place(obj=P.door(w=v["door_w"], h=v["door_h"], open_deg=v["door_open"],
                                 hinge_x=-(v["d_well"] / 2 + 0.030), skin=paint,
                                 liner=well_mat),
                      at=(0.0, 0.0, 0.004))
    if v["tether"]:
