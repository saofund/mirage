...
def cap(d=0.078, flange=0.009, rib_len=0.052, rib_w=0.024, rib_h=0.011, rib_draft=0.66,
    ...
    if rib_slot > 0:
        ...
        p = p.place(obj=frustum(stadium(sl, sw), rib_h - min(rib_slot, rib_h * 0.55), rib_h + 1e-4,
                                1.25, mark="cap_slot"),
...
def door(w=0.175, h=0.165, thick=0.008, open_deg=95.0, hinge_x=-0.10, skin=None, liner=None):
    ...
    body = prism([(-w / 2, -h / 2), (w / 2, -h / 2), (w / 2, h / 2), (-w / 2, h / 2)],
                 0.0, thick, mark="door")
    ...
    # hinged along the door's own -x edge, swung out about the panel's y axis
