...
# the sensor
# --------------------------------------------------------------------------- #
def degrade_depth(depth, rng, sigma_mm=0.42, fill=0.68, flying=0.35, edge_px=1.6,
                  quant_mm=0.0):
    ...
    * **dropout** — everything else that fails: dark surfaces returning no texture, specular
    ...
    jump = grad > 0.004                            # a 4 mm step is a real edge here
...
def _dropout(valid, rgb, rng, fill=0.68):
    """Thin the valid mask down to the measured ROI fill, preferentially in the dark."""
    if fill >= 1.0:
        return valid
    n = int(valid.sum())
    if n == 0:
    ...
    # weight: dark pixels are likelier to drop. Normalised so the mean drop rate is exact.
    w = 1.0 / (0.25 + lum)
    w = w / w[valid].mean()
    keep = rng.random(valid.shape) < 1.0 - np.clip((1 - fill) * w, 0.0, 0.98)
    # a little spatial structure: real dropouts come in blobs, not salt and pepper
    blob = rng.random(valid.shape) < 0.5
    for _ in range(2):
        blob[:, 1:] &= blob[:, :-1]
        blob[1:, :] &= blob[:-1, :]
    return valid & (keep | blob)
...
def make_cloud(rgb, ids, depth, gt, rng, cfg):
    ...
    d, valid = degrade_depth(depth, rng, sigma_mm=cfg["sigma_mm"], flying=cfg["flying"],
                             edge_px=cfg["edge_px"], quant_mm=cfg["quant_mm"])
...
# the loop
# --------------------------------------------------------------------------- #
DEFAULT_CFG = dict(roi_scale=3.9, fill=0.68, sigma_mm=0.42, flying=0.35, edge_px=1.6,
                   quant_mm=0.0, rgb_grain=0.012, rgb_chroma_blur=1.2, rgb_saturation=1.12)
